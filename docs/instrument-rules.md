...
is consistent with a duplicated block and a dropped one. The proof is byte-exact reconstruction of
both halves plus a heading partition, and it is in `docs/reports/FF.md` §2.

**WHEN A RULE MOVES BACK OR ACROSS, THE PIN MOVES WITH IT IN THE SAME BATCH.** Four assertions were
re-pointed at this file when it was created (`check_ec` §2 twice, `test_batch_ce` twice); **FF's
eight blocks needed none — a sweep of every needle any suite or gate asserts into `CLAUDE.md`
found ZERO inside the payload before a byte moved**, which is what makes a re-point necessary
rather than decorative. And
...
- **THE RUNNER WRITES A MANIFEST (`$OUT/.ran`) AND THE DIFFER TRUSTS IT RATHER THAN THE DIRECTORY
  ...
  tree.**
## COUNTS, BANDS, FLAKES AND THE PARSE FLOOR (this file's own heading, FF §1)
**The four bullets below stood inside `CLAUDE.md`'s *Working agreement* and moved here unedited.**
They are the part of that block that binds an INSTRUMENT rather than the game: `baselines.json`,
the battery's count grep, and the parse floor. **The rest of the Working agreement stayed** — the
docs contract, the text standard and the fold rule all bind what the game may contain.
