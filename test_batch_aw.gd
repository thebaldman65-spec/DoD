...
func _spawn(learned: Dictionary, member_patch := {},
	...
	for _i in 20:
		await process_frame
	# Determinism FORCED, not retried (the AK/AL/AR/AS/AT/AU/AV discipline). A
	# driven _resolve still rolls miss, parry AND crit, and a crit is the worst
	# coin to leave live on a spec measured in healing.
