	...
	for i in hero_keys.size():
		...
		if not Run.active_relics.is_empty():
			...
			if not relic_resists.is_empty():
				...
				cfg["resists"] = res
		# ASHEN SKIN (Pyromancer, Inferno row 2 — BATCH BS §3). The fire
		# resistance clause moved down the lane from Kiln-Forged, which now
		# refuses death instead; the SITE is unchanged and for the unchanged
		# reason — resists are a dict entry and payloads only write scalars, so
		# it lands here beside the relic resist block, OUTSIDE the Run.active
		# gate so a standalone sim sees it too. Its other half (a Burn tick he
		# applied heals him) lives in the DoT tick loop.
...
func _make_unit(config: Dictionary, pos: Vector2, tint: Color,
	...
	# BATCH BS §3 — KILN-FORGED's gate. Stamped here for the same reason
	# `blight_cb` is: this is the ONE place every spawned unit passes through,
	# and a hook attached to half the field is a hook a later batch has to
	# remember. A METHOD reference, never a lambda — the count must be read at
	# the instant the killing blow lands, not sampled at the top of a turn.
	u.burning_foes_cb = _burning_foe_count
	# BATCH CE — MANTLE's door. Stamped here for the third time for the same
...
func _update_talent_chips() -> void:
	...
	# OVERBURN's live chip. BATCH BS §2 took the drain out of it with the drain
	# itself — the chip showed two numbers because the spec was a trade and half
	# of it was invisible; there is one number now and it is the bonus. What the
	# chip must still say is that the fire is a BANK: it pays while it stands
	# and pays again when it is spent, which is the spine the drain was bolted
...
func _player_turn(u: BattleUnit) -> void:
	...
	if not u.mod_no_regen:
		if u.resource_name == "Mana":
			...
			u.refresh_bars()
	# BATCH BS §2 — THE OVERBURN BILL USED TO BE CHARGED HERE, immediately after
	# the regen drip, and the ORDER was called the design. Both the call and the
	# function are DELETED: no Mana leaves the Pyromancer at turn start under any
	# burn load, and there is no constant to flip to bring it back. See
	# `_overburn_mult` for the whole argument.
	# BATCH AY §0's SECOND NEW NUMBER: under The Pack, how often TWO beasts
	# are actually standing. A capstone that fields two and then loses one
	# immediately is not the capstone it claims, and only a per-turn sample
	# can tell those apart. Sampled here because this is the one line that
	# runs once per hunter turn whatever else he does.
	if u.the_pack > 0 and not u.is_companion:
		sim_stats["pack_turns"] = sim_stats.get("pack_turns", 0.0) + 1.0
		if _beasts(u).size() >= 2:
			sim_stats["pack_turns_two"] = sim_stats.get("pack_turns_two", 0.0) + 1.0
...
func _bot_drafted_pick(u: BattleUnit) -> Array:
	...
	for ab in u.abilities:
		...
		if ab.target == Ability.Target.ALLY:
			...
			# BATCH CE — MANTLE NEVER LANDS ON AN ALLY ALREADY WEARING A SHIELD.
			# `_grant_divine_shield` merges into a standing barrier (Layered
			# Faith adds to it, `add_status` otherwise maxes it), which would
			# overwrite a live mantle's remaining passes with this cast's — so
			# the bot would spend a turn shortening its own chain. Fortified
			# Spirit's refinement exactly, through a different door.
			if ab.special == "mantle":
				var mn_pool := allies.filter(func(h): \
					return not h.has_status("barrier"))
				if mn_pool.is_empty():
					continue
				return [ab, _lowest_hp(mn_pool)]
		...
		if ab.special in ["suffering", "transference"]:
			...
			if ab.special == "transference":
				...
				if bu_others.is_empty():
					continue
			return [ab, bu_pick]
		# BATCH CE — THREE MORE, and each closes a case where the DEFAULT mark
		# makes the card read as inert in a smoke.
		#
		# REQUIEM consumes ONE enemy's whole pile, so it wants the DEEPEST mark
		# on the field — aimed at `mark` (the lowest-health enemy) it would very
		# often eat two stacks off something about to die and measure nothing.
		if ab.special == "requiem":
			var rq_pick: BattleUnit = null
			var rq_deep := 0
			for rq_e in foes:
				var rq_n: int = rq_e.status_stacks("ruin")
				if rq_n > rq_deep:
					rq_deep = rq_n
					rq_pick = rq_e
			if rq_pick == null:
	...
	match u.hero_key:
		...
		"mage":
			...
			if ashes != null and u.ashes_return <= 0 and not u.ashes_used \
				...
				return [ashes, u]
			# Pyromancer (Batch N loop, Batch AG rework, BATCH BS'S REPLACEMENT
			# RULE). AR's first rule was CONSUME WHEN THE DRAIN EXCEEDS THE
			# REGEN, and §2 deleted the drain — so that rule referenced nothing
			# and had to be replaced rather than merely dropped, or the bot would
			# have kept fire on the board forever and never cashed it.
			# THE HONEST REPLACEMENT: DETONATE THE LARGEST STACK WHEN IT IS WORTH
			# MORE THAN WAITING, AND WILDFIRE WHEN DETONATION IS COOLING. A bank
			# still growing is worth more next turn; one that has reached
			# `DETONATE_AT` is worth cashing now, because burn turns tick away
			# whether he spends them or not. Instrument honesty, not tuning.
			...
			if det != null and ripest != null and ripest_turns >= DETONATE_AT \
					and u.resource >= det.cost and u.ability_ready(det):
				return [det, ripest]
			if wfire != null and not burning_foes.is_empty() \
					and (det == null or not u.ability_ready(det)) \
					and u.resource >= wfire.cost and u.ability_ready(wfire):
				return [wfire, u]
			# BATCH BS §3: Immolate is a DEFENSIVE card now (mitigation plus a
			# retaliation burn), so the bot arms it when there is something to
			# retaliate against rather than when the field is already alight.
			...
			# single-target cash-out on a field it could have eaten.
		...
		"hunter":
			...
			# hunter branch for exactly that reason, and it re-fires the
			# moment a slot empties.
			...
			if bot_beasts.size() < _beast_cap(u):
				...
				# AT CAPACITY: SWAP ONLY WHEN THE INCOMING BOON IS THE BETTER
				# ONE — never merely because the cooldown is up. The margin is
				# what stops it churning, and the comparison runs through
...
func _eff_cost(u: BattleUnit, ab: Ability, target: BattleUnit = null) -> int:
	...
	if u.lone_hunter > 0 and _beasts(u).is_empty():
		c = int(round(c * (1.0 - 0.01 * u.lone_hunter)))
...
func _ability_usable(u: BattleUnit, ab: Ability) -> bool:
	...
	if not u.ability_ready(ab) \
			and not (ab.special == "summon" and u.free_summons > 0):
		return false
...
func _choose_enemy_action(u: BattleUnit) -> Dictionary:
	...
	if not broken_heroes.is_empty():
		...
		# way, scaled by the bond curve — a certainty at a deep enough bond,
		# which is what an uncapped meter is supposed to buy.
...
func _miss_chance(attacker: BattleUnit, defender: BattleUnit = null) -> float:
	...
	if defender != null and defender.has_status("elusive"):
		chance += 0.25
	# HEAT HAZE (Pyromancer, Inferno row 3 — BATCH BS §3). The air off a burning
	# body spoils its own aim: an attacker that is BURNING has a 20% chance to
	# miss him. THE BURNING UNIT IS THE ATTACKER, not the defender — that is the
	# reading his kit can actually produce (he sets things alight; nothing in it
	# sets HIM alight) and it is what makes Immolate one row above it a combo
	# rather than a coincidence. ADDITIVE PERCENTAGE POINTS, like every other
	# term in this function; never a multiplier.
	# WORTH KNOWING AND NOT A BUG: the single-target miss roll is gated `not
	# ab.aoe` at its call site, so this blanks single-target blows only — the
	# same bound Blind has carried since BO.
	if defender != null and defender.heat_haze > 0 \
			and attacker.has_status("burn"):
		chance += 0.01 * defender.heat_haze
	return chance
...
func _resolve(attacker: BattleUnit, ab: Ability, target: BattleUnit, grade: String,
	...
	# missed anyway.
		...
		for hit_i in total_hits:
			...
			# ASH LUNG (Inferno row 7 — BATCH BS §3), the DEALT half. It paid a
			# flat 15% while the drain outran his regen; there is no drain, so it
			# reads the BOARD instead — 4% per burning enemy, both ways,
			# UNCAPPED. The TAKEN half is the mirror of this line in the
			# strike-target block below, and the two must move together: a node
			# that scaled only one way would be half a node.
			if attacker.ash_lung_pct > 0:
				var al_n := _burning_foe_count()
				if al_n > 0:
					raw *= 1.0 + 0.01 * attacker.ash_lung_pct * al_n
			...
			if attacker.is_hero and attacker.passive_id == "pack":
				...
				if attacker.has_status("vengeance"):
					raw *= 1.0 + 0.01 * attacker.vengeance_dmg
				if attacker.lone_hunter > 0 and _beasts(attacker).is_empty():
					raw *= 1.0 + 0.01 * attacker.lone_hunter_dmg
			...
			if strike_target.has_status("unburdened"):
				...
				if strike_target.is_hero:
					_prev(String(strike_target.get_status("unburdened").get(
						"src_name", "")), ub_was - raw)
			# ---------- BATCH BS §3: THE INFERNO LANE, WHICH IS DEFENCE NOW ----------
			# Four of the eight nodes land here, in row order, and they are FOUR
			# DIFFERENT KINDS of mitigation on purpose — flat, retaliatory,
			# scaling-with-the-board, and converting. Each books to `_prev` like
			# every other instrumented reduction, and each is credited to HIM,
			# because every one of them is his own fire keeping him alive.
			# EMBER SHROUD (row 1): flat, on from turn one, gated on ANY enemy
			# burning — the lane's opening statement and the only one of the four
			# that does not scale.
			if strike_target.ember_shroud > 0 and _burning_foe_count() > 0:
				var es_was := raw
				raw *= 1.0 - 0.01 * strike_target.ember_shroud
				if strike_target.is_hero:
					_prev(strike_target, es_was - raw)
			# IMMOLATE (row 4): the re-specced ability's mitigation half. Its
			# retaliation half — whoever strikes him is set Burning — is in the
			# post-strike block, and the two point the same way: the blow is
			# softened AND the attacker walks away alight.
			if strike_target.has_status("immolate"):
				var im_was := raw
				raw *= 1.0 - IMMOLATE_CUT
				if strike_target.is_hero:
					_prev(strike_target, im_was - raw)
			# ASH LUNG (row 7), the TAKEN half — the mirror of the dealt half in
			# the attacker block above. Uncapped, 4% a burning enemy.
			if strike_target.ash_lung_pct > 0:
				var alt_n := _burning_foe_count()
				if alt_n > 0:
					var alt_was := raw
					raw *= maxf(1.0 - 0.01 * strike_target.ash_lung_pct * alt_n, 0.0)
					if strike_target.is_hero:
						_prev(strike_target, alt_was - raw)
			# FORGE BODY (row 8): the row-8 shape BM specified — it READS the
			# accumulated quantity the lane spent seven rows building (burn TURNS
			# on the field, not burning bodies) and CONVERTS it into offence.
			# THE DIFFERENCE BETWEEN THE TWO QUANTITIES IS THE NODE: four enemies
			# at four turns each is 16, and Firestorm alone puts 12-16 on the
			# board in one cast, so the 50% cap needs about fifty burn-turns and
			# will rarely be reached.
			if strike_target.forge_body_pct > 0:
				var fb_turns := _total_burn_turns()
				if fb_turns > 0:
					var fb_was := raw
					raw *= 1.0 - minf(
						0.01 * strike_target.forge_body_pct * fb_turns,
						FORGE_BODY_CAP)
					var fb_stopped := fb_was - raw
					if strike_target.is_hero:
...
# BATCH BS §3 — HOW MANY ENEMIES ARE ALIGHT, which is a DIFFERENT question from
# how many burn TURNS stand and the Inferno lane deliberately asks both. Ember
# Shroud reads "any", Kiln-Forged reads "three or more", Ash Lung reads the
# count itself; Forge Body reads `_total_burn_turns` above. One function each,
# so a node can never be re-pointed at the other quantity by accident — and
# they diverge hard (one enemy at eight turns is 1 here and 8 there).
func _burning_foe_count() -> int:
	var n := 0
	for foe in enemies:
...
# BATCH BS §2 — `_drain_burn_turns` IS DELETED, AND `_total_burn_turns` ABOVE
# IS THE SINGLE DENOMINATOR AGAIN. The second one existed for exactly one
# reason: Ember Debt exempted an enemy from Overburn's Mana DRAIN, and a bill
# with an exemption needs its own total. THE BILL IS GONE (see below), so the
# exemption had nothing to exempt anyone from and the two collapse into one.
# `BattleUnit.ember_debt` went with it; the ability was re-authored.


# OVERBURN, CLAUSE 1 — the reward, and THE ONE PLACE ITS CAP IS DECIDED.
# +2% damage per remaining Burn turn on the enemy team, capped at +40%.
#
# BATCH BS §2 — THE CAP IS FLAT AND NOTHING LIFTS IT ANY MORE, which is a
# change and is said plainly rather than left to be discovered. `_overburn_capped`
# is DELETED with the drain: Heat Shimmer (+20 cap points), Immolate and
# Cauterise-under-20-Mana were the only three lifters, and all three were
# clauses of the Inferno lane the batch re-authored. Raising the payoff's
# ceiling is Detonation's subject, not Inferno's — and every Detonation node
# is already authored, so THE CAP-RAISE WAS DROPPED RATHER THAN REHOMED. A
# later batch wanting one has to author it deliberately, which is the whole
# point of deleting a function instead of leaving it always-true.
#
# THE OLD ASYMMETRY ("the reward caps and the cost does not") DIED WITH THE
# COST. Do not restate it and do not rebuild a cost to justify it.
...
# ---------- BATCH BS §3 — THE INFERNO LANE'S CONSTANTS, IN ONE PLACE ----------
# Every magnitude the lane does NOT hold in a node counter lives here, so a
# reprice is one file and one block. The counters hold the tunable headline of
# each node; these are the terms a node text states but does not scale.
# Immolate's mitigation, read at the strike-target block. The ABILITY's own
# numbers (cost, initiative, cooldown, duration) live in its payload.
const IMMOLATE_CUT := 0.20
# Backblast: the health fraction that arms it, and the Burn it lays on the
# field. Its HEAL is the node's counter (`backblast`), because that is the
# number a reprice would move.
const BACKBLAST_AT := 0.40
const BACKBLAST_BURN := 4
# Forge Body's ceiling, and it is a real ceiling rather than a formality: at
# `forge_body_pct` = 1 it needs about FIFTY burn-turns standing at once.
const FORGE_BODY_CAP := 0.50
# BATCH BS §4 — the bot's cash-out threshold, replacing AR's "the drain exceeds
# the regen". Burn turns on the ripest stack before Detonation is worth more
...
# FORGE BODY'S SECOND HALF (BATCH BS §3): the damage the node PREVENTED is
# dealt to a random BURNING enemy as fire — the lane's whole accumulated
# defence converted into offence, which is what makes it a row 8 rather than a
# fifth mitigation term.
#
# ITS OWN FUNCTION FOR TWO REASONS, both load-bearing. (1) It is called from
# INSIDE `_resolve`'s strike-target block, and `_dmg_frame` is set at
# `_resolve`'s entry — so the frame is SAVED AND RESTORED around the throw or
# every later hit of the same cast books against Forge Body (BL §2's rule about
# re-establishing the frame after a nested resolve, arriving through a new
# door). (2) A rule buried in a 700-line strike block is a rule only a grep can
# check.
#
# IT TARGETS A BURNING ENEMY, NOT ANY ENEMY, and that is the card's text rather
# than a convenience: the mitigation it converts was itself bought by burn
# turns, so there is always one alight when the amount is non-zero. The guard
# survives for the frame in which the last burning enemy has just died.
func _forge_body_throw(hero: BattleUnit, prevented: float) -> void:
	if hero == null or battle_over or prevented <= 0.0:
		return
	var alight := enemies.filter(func(e): return not e.dead and e.has_status("burn"))
	if alight.is_empty():
		return
	var thrown := maxi(int(round(prevented)), 1)
	var mark: BattleUnit = alight.pick_random()
	var was_src := _dmg_src
	var was_label := _dmg_label
	var was_name := _dmg_src_name
	_dmg_frame(hero, "Forge Body", hero.unit_name)
	mark.take_tick_damage(thrown, "-%d" % thrown, Color(1.0, 0.55, 0.25))
	_stat("dmg_hero_%s" % hero.unit_name, thrown)
	_log("   → Talent: Forge Body — the blow it stopped is thrown at %s for %d" % [
...
# BATCH BS §2 — OVERBURN'S THIRD CLAUSE IS DELETED, NOT ZEROED, AND SO ARE THE
# THREE FUNCTIONS THAT SERVED IT: `_overburn_drain`, `_overburn_tick` and
# `_overburn_capped`. There is no constant left for a later batch to flip.
#
# WHAT WAS DELETED: 1 Mana per remaining Burn turn on the enemy team, billed at
# the start of each of his turns, uncapped.
#
# WHY, AND THIS IS THE FINDING RATHER THAN THE CHANGE: seven of the eight
# INFERNO nodes existed to mitigate that one term — Fire Walker reduced it,
# Invigorating Ashes offset it, Immolate doubled it, Kiln-Forged floored it,
# Ash Lung paid him for outrunning it, Cauterise billed it to health and Forge
# Body threw the paid bill at an enemy. A lane whose every node acts on one
# term is one node with eight prices (BC diagnosed the shape, BH proved it),
# and it is worse when the term is a punishment: the lane was not a build, it
# was a payment plan. §3 re-authored the whole column around a new thesis.
#
# THE SPINE SURVIVES THE REMOVAL AND THAT MUST NOT BE FORGOTTEN: his damage is
# still DEFERRED. Burn ticks are small, Detonation pays 250% of the bank, and
# turns spent lighting are turns not killing — the commitment is that he is
# weak until he cashes in. That was always the interesting half; the drain was
# a punishment bolted onto it.
#
# THE 6% GLOBAL BURN TICK CONSTANT IS STILL NOT TOUCHED, for AR's reason: it is
# shared with enemies, runes and every other burn source in the game.


# OVERBURN, CLAUSE 2 — the refund, and THE ONE PLACE IT IS PAID. It is a
...
#
# `LOYALTY_UNCAPPED` is the sentinel `_loyalty_cap` returns by DEFAULT. It is
# not a large number: a large number is a cap a later batch can reach by
# accident, and the whole point of §2 is that partnership never plateaus.
const LOYALTY_UNCAPPED := -1
# The Pack Bond boon's own step, before any node touches it. At 5 Loyalty the
...
# Absolute Devotion + Ancient Pact step that is Loyalty 13). The mitigation is
...
# 0 = no boon. An ACTIVE beast pays the full curve `1 + step x Loyalty`; an
...
# only where a node deliberately imposes one — today that is Wild Rotation
# alone, whose cap IS its cost (`wild_rotation` holds the number).
func _loyalty_cap(hunter: BattleUnit) -> int:
	if hunter.wild_rotation > 0:
		return hunter.wild_rotation
	return LOYALTY_UNCAPPED


# Loyalty: each beast's devotion to the Beastmaster (per beast, NO MAXIMUM).
# +1 every hunter turn that begins with it active and on every summon/
# swap-in (Unbroken Watch adds more on no-damage turns; One Soul and Lone
# Bond double all gains); +5% strike damage per stack (Wild Communion
# deepens it) plus a beast-specific gift, and the Pack Bond boon climbs
...
func _gain_loyalty(hunter: BattleUnit, kind: String, amount := 1) -> void:
	...
	if comp != null:
		...
		comp.refresh_bars()
	# BATCH AY §0's FIRST NEW NUMBER: how deep the bond actually gets now that
	# it has no ceiling. Banked at the gain site rather than at battle end,
	# because Primal Surge and a beast's death both wipe the meter.
	if now > int(sim_stats.get("loyalty_deepest", 0)):
		sim_stats["loyalty_deepest"] = now


# The Loyalty chip lives on the companion (permanent, live counter). Every
# number in it moves with his talents, which is why it is built here and not
# in `unit.add_status` — the same reason AX moved the Ruin chip out.
...
# BATCH CE — ALMS (Holy draft, tranche 3). THE SIXTH STACK GOES SOMEWHERE.
#
# IT SITS BESIDE GRACE RATHER THAN INSTEAD OF IT, and the pair is the whole
# design: one trigger, two effects, DIFFERENT SHEETS. Grace pays HEALTH, which
# a full bar throws away; this pays a BARRIER, which a full bar keeps. Both
# fire on the same spill, which is what stops this being a strictly better
# Grace wearing a draft card's clothes (BD §4).
#
# THE WARD IS DELIBERATELY NOT `divine`: it is Holy's, and a Mercy overflow
# feeding the DEVOUT's Conviction would be one spec's engine leaking out
# through another's card (Magic Barrier's rule, Batch BT, through a new door).
func _alms_spill(cleric: BattleUnit, low_ally: BattleUnit) -> void:
	if not cleric.has_status("alms") or low_ally == null or low_ally.dead:
		return
	var al_power := maxi(int(round(cleric.max_hp * 0.01
		* maxi(cleric.status_power("alms"), 1))), 1)
	_apply_status(low_ally, "barrier", -1, al_power)
	var al_stat: Dictionary = low_ally.get_status("barrier")
	if not al_stat.is_empty():
		al_stat["src"] = cleric.unit_name  # Batch W: absorbs credit the caster
	low_ally.float_text("ALMS %d" % al_power, Color(0.95, 0.85, 0.55))
	_log("   → Alms — the Mercy she cannot hold wards %s (absorbs %d)" % [
...
# 0.00 detonations per battle in trash across 519 battles (deepest mark 10)
# and 0.00 with the ENTIRE Ruin lane force-learned (deepest 7); the boss half
# was 0.13 over 23 fights. THE THRESHOLD WAS NOT THE PROBLEM — GENERATION
# WAS: his corruption accumulated to seven and the payoff sat at ten. One
# number doubles the whole curve, and it lives HERE so that every site which
# is "the passive marking a debuff the Occultist applied" reads one constant
# and node magnitudes (Delirium, Unraveling, Spread of Madness) keep their
...
# EXHAUSTED AND REMOVED — never when Unyielding Aegis re-forms it, because a
# re-formed shield has not broken and the unit side decides that at its one
# site.
#
# THE HOP COUNT IS WHAT BOUNDS THE CHAIN, so there is deliberately NO
# re-entrancy lock here (CB's `_frostbind_mirroring` needed one because that
# recursion had no natural bound; this one strictly decrements and stops).
#
# IT PASSES TO THE LOWEST HEALTH FRACTION OTHER THAN THE BODY IT JUST LEFT.
# Excluding that body is load-bearing rather than tidy: the ally whose shield
# just broke is very often the lowest, so without it the mantle would bounce in
# place and cover one hero three times instead of finding the fight.
#
# A DEAD DEVOUT'S MANTLE FALLS. Every hop is a real Divine Shield and its
...
func _on_barrier_broken(holder: BattleUnit, broke: Dictionary) -> void:
	...
	var devout := _living_devout()
	if devout == null:
		_log("   → Mantle: it falls where it lies — its caster has fallen", "#909090")
		return
	var pick: BattleUnit = null
	for h in heroes:
		if h.dead or h.is_companion or h == holder:
			continue
		if pick == null or float(h.hp) / float(maxi(h.max_hp, 1)) \
				< float(pick.hp) / float(maxi(pick.max_hp, 1)):
			pick = h
	if pick == null:
		_log("   → Mantle: there is nobody left for it to pass to", "#909090")
		return
	var power := maxi(int(broke.get("original", 0)), 1)
	_grant_divine_shield(devout, pick, power)
	var next_stat: Dictionary = pick.get_status("barrier")
	if not next_stat.is_empty():
		next_stat["mantle"] = hops - 1
...
# BATCH AY §9: what a release that consumed NO stacks pays instead. Half the
# step, and it is a SHARE of the base rather than a second magnitude, so the
# two can never drift apart if the base one is ever re-tuned.
const CONVICTION_NO_CONSUME_SHARE := 0.5


func _conviction_growth(devout: BattleUnit, consumed := true) -> void:
	if devout.conviction_base_hp <= 0:
		devout.conviction_base_hp = devout.max_hp - devout.conviction_hp_gained
	var pct := CONVICTION_GROWTH_PCT
	if not consumed:
		pct *= CONVICTION_NO_CONSUME_SHARE
	var step := maxi(int(round(devout.conviction_base_hp * pct)), 1)
	devout.max_hp += step
...
func _resolve_special(attacker: BattleUnit, ab: Ability, target: BattleUnit,
	...
	match ab.special:
		...
		"ember_debt":
			# BATCH BS §4 — RE-AUTHORED, NOT REPLACED. Its whole second sentence
			# was an exemption from Overburn's Mana drain, and §2 deleted the
			# drain; without a bill there was nothing to be free of, and "8 turns
			# of Burn and no clause at all" is not an ability.
			# THE NEW AXIS: PAID BEFORE IT BURNS. Every other Burn payout in the
			# kit EMPTIES the bank — Detonation eats one stack, Wildfire and
			# Cinderfall skim the field — and this one collects Overburn's refund
			# up front while the fire stands its full term. It is the biggest
			# single-target Burn he can apply and the only card that is paid
			# without consuming anything.
			# IT GOES THROUGH `_overburn_refund`, THE ONE DOOR, which is AR's own
		...
		"jubilee":
			...
			# FREQUENCY LOOP straight back — his release would heal, grow the
			# principal (`_conviction_growth`) and roll Communion, which feeds
			...
			if attacker.passive_id != "conviction":
				...
				var jb_heal := maxi(int(round(attacker.max_hp * jb_pct
					* jb_stacks)), 1)
				var jb_got: int = attacker.heal_amount(jb_heal)
				_devout_heal(attacker, jb_got, "jubilee", attacker)
				var jb_mana := maxi(int(round(attacker.max_resource * 0.03
					* jb_stacks)), 1)
				var jb_before := attacker.resource
				attacker.resource = mini(attacker.resource + jb_mana,
					attacker.max_resource)
				var jb_res := attacker.resource - jb_before
				attacker.refresh_bars()
		...
		"mantle":
			if target == null or target.dead or _living_devout() == null:
				...
				# Afterglow, Warded Robes and Sacred Covenant with it. A private
				# barrier would have been a second answer to what a shield is.
				_grant_divine_shield(attacker, target, mn_power)
				var mn_stat: Dictionary = target.get_status("barrier")
				if not mn_stat.is_empty():
					mn_stat["mantle"] = mn_hops
				target.float_text("MANTLE %d" % mn_power, Color(0.95, 0.88, 0.60))
				_sfx("heal", -8.0, 0.8)
		...
		"anathema":
			if target == null or target.dead:
				...
				# not amplify itself. The mark is what the party's NEXT blows
				# read; a self-amplifying opener would make the headline number
				# depend on the order of two lines in this function.
				var an_hit: Dictionary = target.take_hit(0, ab.pressure)
				_stat_bd(attacker, ab.pressure)
		...
		"requiem":
			...
			if target == null or target.dead or rq_stacks <= 0:
				_log("%s: Requiem — there is no mark on it to sing out" % \
					attacker.unit_name, "#909090")
			else:
				var rq_per := 0.10 if is_perfect else REQUIEM_PER_STACK
				# THE PILE IS SPENT BEFORE THE BLOW LANDS, and that ordering is
				# a decision rather than a convenience: Ruin is +2% damage taken
				# a stack (+5% under Deeper Hex), so striking through the mark
				# the card is made of would make the damage QUADRATIC in the
				# stack count — the squaring trap arriving through the damage
				# door rather than the Break one.
				#
				# THE PRIMER GOES WITH IT. A mark sitting primed at ten would
				# otherwise detonate for a full 90% of Attack at its next turn
				# off a pile that no longer exists.
				target.remove_status("ruin_primed")
				target.remove_status("ruin")
				target.weight_of_ruin = 0
				var rq_raw := rq_per * rq_stacks * attacker.attack * mult \
					* randf_range(0.9, 1.1)
				rq_raw *= 1.0 - float(target.resists.get("shadow", 0.0))
				var rq_final := maxi(int(round(rq_raw
					* (1.0 - target.effective_armor()))), 1)
				var rq_hit: Dictionary = target.take_hit(rq_final, ab.pressure)
				_stat("dmg_hero_" + attacker.unit_name, rq_final)
				_stat_bd(attacker, ab.pressure)
				target.float_text("%d" % rq_final, Color(0.75, 0.45, 0.85))
				var rq_share := maxi(int(round(attacker.max_hp * 0.02
					* rq_stacks)), 1)
				# BOOKED AS WHAT LANDED RATHER THAN WHAT WAS ASKED FOR. An ally
				# at full health takes none of it, and printing the headline
				# would teach a player a rule the fight did not follow (BT's
				# Inner Arcane lesson, BC's Blessed Barrier before it).
				var rq_paid := 0
				var rq_total := 0
				for rq_h in heroes:
					if rq_h.dead or rq_h.is_companion:
						continue
					var rq_got: int = rq_h.heal_amount(rq_share, rq_h != attacker)
					if rq_got <= 0:
						continue
					rq_paid += 1
					rq_total += rq_got
					rq_h.float_text("+%d" % rq_got, Color(0.7, 0.4, 0.9))
		...
		"instinct":
			...
			var hi_shots := attacker.instinctive if attacker.instinctive > 0 else 3
			var hi_info: Array = STATUS_INFO["instinct"]
			...
			for bw_comp in _beasts(attacker):
				...
				if attacker.devoted_fury > 0:
					bw_turns += int(attacker.loyalty.get(
						bw_comp.companion_kind, 0)) * attacker.devoted_fury
		...
		"spirit_bond":
			...
			if not sb_beasts.is_empty():
				...
				var sb_mana := int(attacker.max_resource \
					* (0.15 + 0.01 * attacker.deep_reserves_ranks))
		...
		"immolate":
			# BATCH BS §3 — RE-SPECCED WITH ITS LANE, KEEPING ITS ID AND ITS
			# ABILITY SLOT. It loses BOTH Overburn clauses: the drain-doubling
			# went with the drain, and the "no damage cap" half went because the
			# cap is no longer this lane's business (§3 — it is Detonation's, and
			# it was dropped rather than rehomed). What remains is RETALIATION
		...
		"holy_heal":
			...
			# BATCH CE — INDEPENDENT `if` RATHER THAN `elif`, AND IT IS A
...
# kind's own read site decides its base worth; `_bond_mult` supplies the
...
func _bot_boon_worth(hunter: BattleUnit, kind: String) -> float:
	...
	match kind:
		"canis":
			...
			var wounded := enemies.filter(func(e): return not e.dead \
				and e.hp < e.max_hp * 0.35).size()
			return 0.15 * curve * maxf(float(wounded), 0.5)
		"ursus":
			# 10% less damage taken, and it draws the blows — worth double
			# while the hunter himself is in danger.
			return 0.10 * curve * (2.0 if hunter.hp < hunter.max_hp * 0.5 else 1.0)
		"aguila":
...
#
# BATCH AY §1 — LONE BOND WINS OVER THE PACK, AND THIS IS THE ONE PLACE THAT
# IS DECIDED. "One beast per fight" plus "two beasts at once" is the single
# combination that must be impossible, and ROW EXCLUSIVITY DOES NOT PREVENT
# IT: Lone Bond is Devotion row 7 and The Pack is a row-8 capstone with no
# lane-purity requirement, so a player can hold both. Rather than build a
# picker-level exclusion subsystem (§11 forbids that), the contradiction is
# resolved where the number is actually read, and BOTH node descriptions say
# so — the door is visible before the point is spent.
...
# ONE SOUL, THE ONE WRITER. The bond is the hunter plus every beast he
# currently fields — three bodies under The Pack — and every member holds the
# same list, so `take_hit` can divide a wound across all of them without
# asking who is whose partner. Called wherever the field changes.
func _sync_soul_bond(hunter: BattleUnit) -> void:
	if hunter == null:
		return
	if hunter.one_soul <= 0:
		hunter.soul_bond = []
		return
	var bond: Array = [hunter]
	bond.append_array(_beasts(hunter))
	hunter.soul_bond = bond
	for b in _beasts(hunter):
		b.soul_bond = bond
...
func _do_summon(hunter: BattleUnit, kind: String, target: BattleUnit = null) -> void:
	...
	if hunter.free_summons > 0:
		...
		if hunter.no_beast_left_loyalty > int(hunter.loyalty.get(kind, 0)):
			hunter.loyalty[kind] = hunter.no_beast_left_loyalty
	...
	# Shared Devotion: the whole pack feels the call, the arriving beast
	# included — "not only the arriving one" is the node's whole sentence.
	if hunter.shared_devotion > 0:
		for k2 in ["ursus", "canis", "aguila"]:
			_gain_loyalty(hunter, k2, hunter.shared_devotion)
	_stamp_loyalty_chip(hunter, comp)
	await _wait(0.5)
	...
	match kind:
		...
		"canis":
			...
			_log("%s: Bloodhowl — 15 Bleed to every enemy" % caller_name,
				"#e05050")
			# Herald widens it to the `herald` bloodiest enemies.
			var howl_live: Array = enemies.filter(func(en): return not en.dead)
			var bloodiest: Array = howl_live.duplicate()
			bloodiest.sort_custom(
				func(a, b): return a.bleed_buildup > b.bleed_buildup)
			bloodiest = bloodiest.slice(0, hunter.herald)
			for e in howl_live:
				_add_bleed_with_burst(e, 30 if e in bloodiest else 15)
			if not bloodiest.is_empty():
				_log("   → Herald: the howl bleeds %s twice as deep" % \
					", ".join(bloodiest.map(func(e): return e.unit_name)),
					"#e05050")
		"aguila":
			...
			for _extra in hunter.herald:
				var others := enemies.filter(
					func(e): return not e.dead and not dives.has(e))
				if others.is_empty():
					break
...
func _companion_hit(comp: BattleUnit, victim: BattleUnit, dmg: float, pr: int,
	...
	if pm != null and not pm.dead:
		var fed := 0
		if pm.symbiosis > 0:
			fed += int(pm.max_resource * 0.01 * pm.symbiosis)
...
func _on_beast_death(comp: BattleUnit) -> void:
	...
	if pm.steadfast_bond > 0 and had > 0:
		...
		# reads the Loyalty that survived, so the two nodes compose.
		pm.loyalty[kind] = had * pm.steadfast_bond / 100
		_log("   → Steadfast Bond: %s's Loyalty endures at %d" % [
			comp.unit_name, pm.loyalty[kind]], "#c08850")
	elif had > 0:
		pm.loyalty[kind] = 0
	...
	if pm.vengeance > 0 and not pm.dead:
		...
		_log("   → Vengeance: %s inherits the %s bond at full strength — +%d%% damage, for the rest of the battle" % [
...
# BATCH AY §0 — THE BEASTMASTER'S TWO NEW NUMBERS. The deepest Loyalty reached
# on a single beast (§2 took its ceiling away, so "how deep does it actually
# get" stopped being answerable from the design), and — under The Pack only —
# the share of the hunter's turns on which TWO beasts were genuinely standing.
# Shared by the standalone report and RunSim's, the ruin_report_line pattern.
# Returns "" when no Beastmaster stood: a zero on a party without one reads as
# a broken instrument rather than a finding.
static func pack_report_line(stats: Dictionary) -> String:
	var deepest: int = int(stats.get("loyalty_deepest", 0))
	var pt: float = stats.get("pack_turns", 0.0)
	if deepest <= 0 and pt <= 0.0:
		return ""
...
func _on_damage_taken(victim: BattleUnit, lost: int, hp_before: int) -> void:
	...
	# Burn tick, a recoil or a bane alike, and a future damage source cannot
	# forget to arm it. It is ABOVE the `sim` guard below ON PURPOSE — the
	# LEDGER is real-play-only, but a TALENT that fired only outside the sim is
	# a talent no measurement in this project could ever see (BL's own dropped
	# `sim and` guard in the DoT loop is the precedent, from the other side).
	_backblast_check(victim)
	# Real play only from here. RunSim keeps its own statistics path and must
	# never double-count (the rule that keeps the harness honest); a beast
...
# first time he is put under 40% health, the furnace vents — every enemy is set
# Burning and he takes back 15% of his maximum. It is the lane's EMERGENCY, the
# one row that does nothing at all until the fight goes wrong.
#
# TWO GUARDS THAT ARE NOT INTERCHANGEABLE. `backblast_used` is what makes "once
# per battle" true (a threshold check alone would re-fire on every hit taken
# while he sits under 40%). `hp > 0` is what stops a dead Pyromancer venting: it
# is called BELOW all four death refusals, so a hero Kiln-Forged or Ashes of
# Al'ar just saved at 1 HP DOES get it — which is correct, he is alive and very
# far under the line — while one who actually fell does not.
func _backblast_check(u: BattleUnit) -> void:
	if u.backblast <= 0 or u.backblast_used or u.dead or u.hp <= 0:
		return
	if u.hp > int(u.max_hp * BACKBLAST_AT):
		return
	...
	for foe in enemies:
		if foe.dead:
			continue
		_apply_status(foe, "burn", BACKBLAST_BURN, 0, _dot_tick("burn", u), u)
		bb_lit += 1
	_sfx("bomb", -8.0, 0.9)
	_message("%s's furnace vents!" % u.unit_name)
	_log("   → Talent: Backblast — %s sets %d enem%s alight (%d turns) and takes back %d" % [
		u.unit_name, bb_lit, "y" if bb_lit == 1 else "ies", BACKBLAST_BURN,
		bb_heal], "#b0a8e0")
