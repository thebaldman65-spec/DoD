...
func _pools() -> void:
	...
	ok(spec_total == 24, "§4: SPEC_DRAFT_POOLS is untouched at BP's twenty-four (%d)" % spec_total)
	ok(spec_total + total == 48, "§0: the draft holds 48 of a target ~96 (%d)" % (spec_total + total))
	for sp2 in Classes.SPEC_DRAFT_POOLS:
		ok(Classes.spec_draft_pool(sp2).size() == 2,
			"§0: %s's SPEC pool is still two deep — tranches 2 and 3 are owed" % sp2)
...
func _docs() -> void:
	...
	ok(master.contains("Batch BR"), "§5: master.html is stamped Batch BR")
