...
func _pools() -> void:
	...
	ok(total == 125,
		"the spec pools hold 125 (CI's 96, DO's twenty-two, DR's net +1, DS's six), got %d"
			% total)
...
func _docs() -> void:
	...
	# threshold, so CX cut it at the CN/CO boundary: Batch BT — with everything
	# from BP to CN — moved OUT OF THE REPO into `changelog-archive.html`. The old
	# `contains("Batch BT")` would have gone on PASSING against the live file,
	...
	ok(not live_log.contains("<h2>2026-08-14 &mdash; Batch BT"),
