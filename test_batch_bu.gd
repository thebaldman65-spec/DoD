...
func _spawn(spec: String, lineup: Array, learned := {}) -> Node:
	...
	var specs := ["berserker", "arcanist", spec, "sharpshooter"]
	...
	# count AND peak. A check that set Faith after twenty frames and then
	# awaited a cast would have its values wiped out from under it and read as a
