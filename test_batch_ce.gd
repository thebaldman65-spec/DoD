...
func _pools() -> void:
	...
	# card on two shelves — is asked by `_names()` below ("is not also a spec draft
	# card", over every class-wide card), which HG's control c2 turned red and HH
	...
	var gp := Fixture.one_pool_a_class("test_batch_ce, the BQ/BR/BT/CB leak arm (a NINE card on a class-wide shelf)")
	ok(bool(gp[0]), String(gp[1]))
	print("  [record] the BQ/BR/BT/CB leak arm, retired at HH §2 onto `_names()`: %d of %d NINE cards sit on a class-wide shelf %s" % [
...
func _names() -> void:
	...
	# `test_batch_cb`'s whole-draft sweeps — what `test_batch_bp` §5's tranche arm and
