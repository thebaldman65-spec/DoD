...
func _pools() -> void:
	# **FOLDED BY BATCH HD §3.** Three loops stood here asking each lineage's
	# SHELF for at least eight — the Hunter three this suite shipped (2 -> 5 here,
	# 5 -> 8 at CH), the nine beside them, and the Warrior three whose debt this
	# suite kept visible until BW and CI paid it. The question — *did a pool
	# quietly empty* — is kept; what moved is what a pool IS. Since GP a hero
	...
	# no-engine hero is offered. The helper the twelve suites share asks both
	# halves of each class's pool at the floors HD measured; it also takes the
	# class-wide shelves' floor of three from further down. BV's own five are
	# still pinned as the first of each shelf by the literals below.
	for fl in Fixture.class_pool_floors("test_batch_bv pools"):
	...
	ok(total >= 125, "the spec pools have FALLEN to %d, below the 125 that shipped" % total)
	...
	ok(draft_total >= 149,
		"the whole draft has FALLEN to %d, below the 149 that shipped" % draft_total)
...
func _docs() -> void:
	...
	ok(master.contains("%d of " % dy_draft),
		"master.html states the new draft count (%d)" % dy_draft)
	...
	# threshold, so CX cut it at the CN/CO boundary: Batch BV — with everything
	# from BP to CN — moved OUT OF THE REPO into `changelog-archive.html`. The old
	# `contains("Batch BV")` would have gone on PASSING against the live file,
	...
	ok(not live_log.contains("<h2>2026-08-14 &mdash; Batch BV"),
		"CX moved this batch's entry OUT of the live changelog")
	ok(chlog.contains("<h2>2026-08-14 &mdash; Batch BV"),
		"...and the archive carries the Batch BV entry")
