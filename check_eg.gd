...
func _s1_ladder_live() -> void:
	...
	for m in run.party:
		...
		# BATCH GN — and the class kit's three count too, less any the lineage
		# already names, so "seated at the cap" is measured off the live opening.
		var want: int = run.ability_slot_cap() - Classes.lineage_slots(spec) \
			- Classes.kit_slots(String(m["key"]), spec)
		m["bm_abilities"] = []
		m.erase("bm_equipped")
...
func _s2_pool_and_loadout() -> void:
	...
	# BATCH GN — the core is the lineage's count and the class kit's.
	var core: int = Classes.lineage_slots("swordmaster") \
		+ Classes.kit_slots("warrior", "swordmaster")
	ok(int(run.ability_slots_used(m)) == core + 2,
	...
	ok(int(run.ability_slots_used(m)) == core + 1,
